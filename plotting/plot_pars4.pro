...
lls=['!8C!D1.0!N!X',ds+'1.2!N',ds+'1.5!N','!8r!D1.2!N!X','!8<R>!D1.25!N!X','!8<R>!D1.6!N!X','!8<R>!D1.0!N!X',$
     '!8C!D1.6!N!X',dds+'1.0!N',dds+'1.2!N','!8sl!D0.55!N!X','!8R!D1.04!N!X','!8r!D1.0!N!X','!8r!D0.6!N!X','!8sl!D1.6!N!X']
pos=[[3,-0.2],[0,-1],[8,9.9],[5,2.8],[0,0.05],[10,0.07],[0,0.13],[13,4],[-1,0.02],$
     [2,0.04],[0,-3.08],[0,0.13],[2,1.35],[2,2.6],[0,0.015]]
...
tau=taus
 for p=0, 14 do begin
  ...
  yr=[min(avg[0:19,[09,19,24],*,p]-std[0:19,[09,19,24],*,p],/nan)*0.98,$
      max(avg[0:19,[09,19,24],*,p]+std[0:19,[09,19,24],*,p],/nan)*1.02]
  if p eq 10 then begin
   yr[1]=max(avg[0:19,[09,19,29],*,p]+std[0:19,[09,19,29],*,p],/nan)*0.96 & std[*,*,*,p]=std[*,*,*,p]/2.
  endif
  if p eq 14 then begin
   yr=[-0.002,0.030]
   for i=0,19 do for j=0,1 do avg[*,i,j,p]=smooth(avg[*,i,j,p],10)
   for i=0,19 do for j=0,1 do std[*,i,j,p]=smooth(std[*,i,j,p],10) 
 ...
 if p gt 10 then plot, tau,avg[*,09,0,p],xtitle='Optical thickness',xrange=xra,$
  ...
  pl=avg[0:mt,09,0,p]+std[0:mt,09,0,p] & ml=avg[0:mt,09,0,p]-std[0:mt,09,0,p]
  lp=where(pl gt yr[1],nlp) & if nlp gt 0 then pl[lp]=yr[1]
  lm=where(ml lt yr[0],nlm) & if nlm gt 0 then ml[lm]=yr[0]
  polyfill,[tau[0:mt],reverse(tau[0:mt])],[ml,reverse(pl)],color=201,$
   clip=[1,yr[0],100,yr[1]],spacing=0.06,orientation=-45.
  oplot,tau,avg[*,09,0,p],thick=7
  pl=avg[0:mt,09,1,p]+std[0:mt,09,1,p] & ml=avg[0:mt,09,1,p]-std[0:mt,09,1,p] 
  lp=where(pl gt yr[1],nlp) & if nlp gt 0 then pl[lp]=yr[1] 
  lm=where(ml lt yr[0],nlm) & if nlm gt 0 then ml[lm]=yr[0]
  polyfill,[tau[0:mt],reverse(tau[0:mt])],[ml,reverse(pl)],color=201,$
  ...
  pl=avg[0:mt,19,0,p]+std[0:mt,19,0,p] & ml=avg[0:mt,19,0,p]-std[0:mt,19,0,p] 
  lp=where(pl gt yr[1],nlp) & if nlp gt 0 then pl[lp]=yr[1]
  lm=where(ml lt yr[0],nlm) & if nlm gt 0 then ml[lm]=yr[0] 
  polyfill,[tau[0:mt],reverse(tau[0:mt])],[ml,reverse(pl)],color=71,$
   clip=[1,yr[0],100,yr[1]],spacing=0.06,orientation=-45.
  oplot,tau,avg[*,19,0,p],color=70,thick=7
  pl=avg[0:mt,19,1,p]+std[0:mt,19,1,p] & ml=avg[0:mt,19,1,p]-std[0:mt,19,1,p] 
  lp=where(pl gt yr[1],nlp) & if nlp gt 0 then pl[lp]=yr[1]
  lm=where(ml lt yr[0],nlm) & if nlm gt 0 then ml[lm]=yr[0] 
  polyfill,[tau[0:mt],reverse(tau[0:mt])],[ml,reverse(pl)],color=71,$
   clip=[1,yr[0],100,yr[1]],spacing=0.08,orientation=45.
  ...
  pl=avg[0:mt,29,0,p]+std[0:mt,29,0,p] & ml=avg[0:mt,29,0,p]-std[0:mt,29,0,p] 
  lp=where(pl gt yr[1],nlp) & if nlp gt 0 then pl[lp]=yr[1]
  lm=where(ml lt yr[0],nlm) & if nlm gt 0 then ml[lm]=yr[0] 
  polyfill,[tau[0:mt],reverse(tau[0:mt])],[ml,reverse(pl)],color=251,$
   clip=[1,yr[0],100,yr[1]],spacing=0.06,orientation=-45.
